...
class WindowCoveringTilt extends HandlerPattern {
	...
	onKNXValueChange(field, oldValue, knxValue) {
		...
		if (field === "TargetPosition") {
			...
			switch (knxValue) {
				case 0:
					this.myAPI.setValue("TargetPosition", 1); // top position, so home shows "opening"
	...
	onHKValueChange(field, oldValue, newValue) {
		...
		if (field === "TargetPosition") {
			...
			if (newValue === 0){
				this.myAPI.setValue("TargetHorizontalTiltAngle", 1); // set shutters to closed as well
				this.onHKValueChange("TargetHorizontalTiltAngle", 0, 1);
			}
			...
			console.log('INFO: onHKValueChange after calc ('  + knxValue+ ")");
			this.myAPI.knxWrite("TargetHorizontalTiltAngle", knxValue, "DPT1"); // send the new position to the KNX bus
